				...
				if($_SERVER["REQUEST_METHOD"] == "POST")
				{
					if(!isset($_POST['par1']) || !isset($_POST['par4']))
					{
						...
						exit();
							...
							$parent_id_attribute 	= intval(trim($_POST['par1']));
                            $id_attribute           = intval(trim($_POST['par4']));

							if(!empty($parent_id_attribute) && !empty($id_attribute))
		                    {
		                    	require_once($slash.'controllers/functions/attributesController.php');

								attributesController::registerParentAttribute($parent_id_attribute,$id_attribute);
							}else{
									$valor = array("estado" => "false",
