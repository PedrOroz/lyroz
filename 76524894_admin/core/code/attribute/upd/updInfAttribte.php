    ...
    try
    {
        ...
        if(!isset($_SESSION['id_user_dao']) || empty($_SESSION['id_user_dao'])){
            ...
            exit();
                ...
                {
                    if(!isset($_POST['par1']) || !isset($_POST['title_attribute_lang']))
                    {
                        $valor = array("estado" => "false",
                        ...
                        exit();
                            ...
                            $id_attribute_lang      = intval(trim($_POST['par1']));
                            $title_attribute_lang   = addslashes(trim($_POST['title_attribute_lang']));

                            if(!empty($id_attribute_lang) && !empty($title_attribute_lang))
                            {
                                require_once($slash.'controllers/functions/attributesController.php');

                                attributesController::updateInformationAttribute($id_attribute_lang,$title_attribute_lang);
                            }else{
                                    $valor = array("estado" => "false",
