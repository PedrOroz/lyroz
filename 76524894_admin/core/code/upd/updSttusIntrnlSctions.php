				...
				if($_SERVER["REQUEST_METHOD"] == "POST")
				{
					if(!isset($_POST['par1']) || !isset($_POST['par2']) || !isset($_POST['par3']) || !isset($_POST['par4']))
					{
						...
						exit();
							...
							$id_type_section 	= intval(trim($_POST['par4']));
							$id_call 			= intval(trim($_POST['par5']));

							$title				= addslashes(trim($_POST['par2']));

							//EN ESTE CASO NO ES NECESARIO VALIDAR par3 (s_table) POR QUE SU VALOR PUEDE SER 0
							if(!empty($id_table) && !empty($id_type_section) && !empty($id_call) && !empty($title))
							{
								require_once($slash.'models/cfg/conectorDB.php');

								$switch_status		= intval(trim($_POST['par3']));

								ConectorDB::updateStatusInternalSections($id_table,$title,$switch_status,$id_type_section,$id_call);
							}else{
									$valor = array("estado" => "false",
