	...
	try
	{
        ...
        if(!isset($_SESSION['id_user_dao']) || empty($_SESSION['id_user_dao'])){
            ...
            exit();
				...
				{
					//NO ES NECESARIO VALIDAR $_POST['par3'] YA QUE SU VALOR PUEDE SER 0
					if(!isset($_POST['par1']) || !isset($_POST['par2']) || !isset($_POST['par4']))
					{
						$valor = array("estado" => "false",
						...
						exit();
							...
							if(!empty($id_table) && !empty($id_call) && !empty($title))
							{
								require_once($slash.'models/cfg/conectorDB.php');

								$switch_status_visible	= intval(trim($_POST['par3']));

								ConectorDB::updateGeneralStatusVisible($id_table,$title,$switch_status_visible,$id_call);
							}else{
									$valor = array("estado" => "false",
