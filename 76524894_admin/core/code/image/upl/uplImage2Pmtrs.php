    ...
    try
    {
        ...
        if(!isset($_SESSION['id_user_dao']) || empty($_SESSION['id_user_dao'])){
            ...
            exit();
                ...
                {
                    if(!isset($_POST['par1']) || !isset($_POST['par2']) || !isset($_POST['par3']) || !isset($_POST['par4']) || !isset($_POST['id_image_section']))
                    {
                        $valor = array("estado" => "false",
                        ...
                        exit();
                            ...
                            if(!empty($_FILES['fileInputUploadImage2Parameters']['name']))
                            {
                                $id_table               = intval(trim($_POST['par1']));
                                $id_table_lang          = intval(trim($_POST['par2']));
                                $id_type_image          = intval(trim($_POST['par4']));
                                $id_image_section       = intval(trim($_POST['id_image_section']));

                                $title_table_lang       = addslashes(trim($_POST['par3']));
                                $file_name              = trim($_FILES['fileInputUploadImage2Parameters']['name']);

                                if(!empty($id_table) && !empty($id_table_lang) && !empty($title_table_lang) && !empty($id_type_image) && !empty($id_image_section) && !empty($file_name))
                                {
                                    require_once($slash.'controllers/functions/imageController.php');

                                    $file_error         = $_FILES["fileInputUploadImage2Parameters"]["error"];
                                    $file_type          = $_FILES["fileInputUploadImage2Parameters"]["type"];
                                    $file_tmp_name      = $_FILES["fileInputUploadImage2Parameters"]["tmp_name"];
                                    $file_size          = $_FILES["fileInputUploadImage2Parameters"]["size"];

                                    imageController::uploadImage2Parameters($id_table,$id_table_lang,$title_table_lang,$id_type_image,$id_image_section,$file_error,$file_name,$file_type,$file_tmp_name,$file_size);
                                }else{
                                        $valor = array("estado" => "false",
