        ...
        if(!isset($_SESSION['id_user_dao']) || empty($_SESSION['id_user_dao'])){
            ...
            exit();
                ...
                if($_SERVER["REQUEST_METHOD"] == "POST")
				{
					...
					if(!isset($_POST['name_user']) || !isset($_POST['last_name_user']) || !isset($_POST['id_role']) || !isset($_POST['gender_user']) || !isset($_POST['email_user']) || !isset($_POST['emailConfirmation']) || !isset($_POST['password1']) || !isset($_POST['password2']))
					{
						$valor = array("estado" => "false",
						...
						exit();
							...
							$id_role 					= intval(trim($_POST['id_role']));
							...
							if(!empty($id_role) && !empty($name_user) && !empty($last_name_user) && !empty($gender_user) && !empty($email_user) && !empty($emailConfirmation) && !empty($password1) && !empty($password2))
		                    {
								...
								$id_social_media 		= intval(trim($_POST['id_social_media']));

								$birthdate_user 		= (isset($_POST['birthdate_user']) && !empty($_POST['birthdate_user']) ? filter_input(INPUT_POST, str_replace_string(array("'", '"'),"",trim($_POST['birthdate_user']))) : NULL);
								...
								if(!empty($_POST['filters_user'])){
									$filters_user_input   = $_POST['filters_user'];
	                    			for ($i=0; $i < count($filters_user_input); $i++)
									{
										$filters_user .=  trim($filters_user_input[$i]).($i <= (count($filters_user_input) - 2) ? ', ' : '');
									}
								}

								$url_user_social_media	= addslashes(trim(filter_input(INPUT_POST, 'url_user_social_media', FILTER_SANITIZE_URL)));
								
		                    	if(empty($id_social_media) || empty($url_user_social_media))
		                    	{
		                    		//REGISTRAR SOLO USUARIO
		                    		userController::registerUser($id_role,$name_user,$last_name_user,$rfc_user,$curp_user,$membership_number_user,$about_me_user,$biography_user,$birthdate_user,$age_user,$gender_user,$lada_telephone_user,$telephone_user,$lada_cell_phone_user,$cell_phone_user,$ship_address_user,$address_user,$country_user,$state_user,$city_user,$municipality_user,$colony_user,$cp_user,$street_user,$outdoor_number_user,$interior_number_user,$between_street1_user,$between_street2_user,$other_references_user,$nationality_user,addslashes($filters_user),$username_website,$email_user,$emailConfirmation,$password1,$password2);
		                    	}else{
		                    			//REGISTRAR USUARIO CON REDES SOCIALES
		                    			userController::registerUserWithSocialNetwork($id_role,$name_user,$last_name_user,$rfc_user,$curp_user,$membership_number_user,$about_me_user,$biography_user,$birthdate_user,$age_user,$gender_user,$lada_telephone_user,$telephone_user,$lada_cell_phone_user,$cell_phone_user,$ship_address_user,$address_user,$country_user,$state_user,$city_user,$municipality_user,$colony_user,$cp_user,$street_user,$outdoor_number_user,$interior_number_user,$between_street1_user,$between_street2_user,$other_references_user,$nationality_user,addslashes($filters_user),$username_website,$email_user,$emailConfirmation,$password1,$password2,$id_social_media,$url_user_social_media);
		                    		 }
							}else{
									$valor = array("estado" => "false",
