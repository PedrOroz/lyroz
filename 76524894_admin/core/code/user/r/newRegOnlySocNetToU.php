				...
				if($_SERVER["REQUEST_METHOD"] == "POST")
				{
					if(!isset($_POST['par1']) || !isset($_POST['id_social_media']) || !isset($_POST['url_user_social_media']))
					{
						...
						exit();
							...
							if(!empty($id_user) && !empty($id_social_media) && !empty($url_user_social_media))
		                    {
		                    	require_once($slash.'controllers/functions/userController.php');

		                    	userController::registerOnlySocialNetworkToUser($id_user,$id_social_media,$url_user_social_media);
							}else{
									$valor = array("estado" => "false",
