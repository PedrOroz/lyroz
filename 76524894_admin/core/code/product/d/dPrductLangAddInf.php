                ...
                if($_SERVER["REQUEST_METHOD"] == "POST")
                {
                    if(!isset($_POST['par1']) || !isset($_POST['par2']) || !isset($_POST['par3']))
                    {
                        ...
                        exit();
                            ...
                            $tag_product_lang_additional_information    = addslashes(trim($_POST['par2']));

                            if(!empty($id_product_lang_additional_information) && !empty($tag_product_lang_additional_information) && !empty($id_type_image))
                            {
                                require_once($slash.'controllers/functions/productsController.php');

                                productsController::deleteProductAdditionalInformation($id_product_lang_additional_information,$tag_product_lang_additional_information,$id_type_image);
                            }else{
                                    $valor = array("estado" => "false",
