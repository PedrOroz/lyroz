                ...
                if($_SERVER["REQUEST_METHOD"] == "POST")
                {
                    if(!isset($_POST['par1']) || !isset($_POST['par2']) || !isset($_POST['par3']))
                    {
                        ...
                        exit();
                            ...
                            $title_product_lang_promotion  = addslashes(trim($_POST['par2']));

                            if(!empty($id_product_lang_promotion) && !empty($id_type_image) && !empty($title_product_lang_promotion))
                            {
                                require_once($slash.'controllers/functions/productsController.php');

                                productsController::deleteProductPromotion($id_product_lang_promotion,$id_type_image,$title_product_lang_promotion);
                            }else{
                                    $valor = array("estado" => "false",
