    ...
    try
    {
        ...
        if(!isset($_SESSION['id_user_dao']) || empty($_SESSION['id_user_dao'])){
            ...
            exit();
				...
				{
					if(!isset($_POST['par1']) || !isset($_POST['id_type_promotion']) || !isset($_POST['price_discount_product_lang_promotion']) || !isset($_POST['discount_rate_product_lang_promotion']) || !isset($_POST['start_date_product_lang_promotion']))
					{
						$valor = array("estado" => "false",
						...
						exit();
							...
							$id_product_lang_promotion                		= intval(trim($_POST['par1']));
							...
							if(!empty($id_product_lang_promotion) && !empty($id_type_promotion) && !empty($price_discount_product_lang_promotion) && !empty($discount_rate_product_lang_promotion) && !empty($start_date_product_lang_promotion)){
								...
								productsController::updateInformationProductPromotion($id_product_lang_promotion,$id_type_promotion,$title_product_lang_promotion,$sku_product_lang_promotion,$price_discount_product_lang_promotion,$discount_rate_product_lang_promotion,$description_small_product_lang_promotion,$description_large_product_lang_promotion,$link_product_lang_promotion,date("Y-m-d", $start_date_product_lang_promotion),date("Y-m-d", $finish_date_product_lang_promotion));
							}else{
									$valor = array("estado" => "false",
