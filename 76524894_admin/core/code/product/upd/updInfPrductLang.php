    ...
    try
    {
        ...
        if(!isset($_SESSION['id_user_dao']) || empty($_SESSION['id_user_dao'])){
            ...
            exit();
				...
				{
					//EN ESTE CASO SI ES NECESARIO EL PAR5 (id_product_lang) PERO NO EL PAR1 (id_product)
					if(!isset($_POST['par5']) || !isset($_POST['par6']) || !isset($_POST['par7']) || !isset($_POST['par8']) || !isset($_POST['id_type_of_currency']) || !isset($_POST['id_tax_rule']))
					{
						$valor = array("estado" => "false",
						...
						exit();
							...
							if(!empty($id_product_lang) && !empty($id_type_product) && !empty($id_type_of_currency) && !empty($id_tax_rule) && !empty($title_product_lang) && !empty($friendly_url_product_lang)){
		                    	...
		                    	$general_price_product_lang = (isset($_POST['general_price_product_lang']) && !empty($_POST['general_price_product_lang']) ? filter_input(INPUT_POST, 'general_price_product_lang', FILTER_VALIDATE_FLOAT) : 0.00);
	                    		...
	                    		//$id_type_info
							    	...
							    	//21 = meta_keywords_product_lang
							    																				//$id_product_lang,$id_type_info
	                    		$general_price_product_lang_old = productsController::showSpecificInformationByProductLangId($id_product_lang,3);
								...
								$INT_background_color_degraded_product_lang	= intval(trim($_POST['background_color_degraded_product_lang']));
							    																				//$id_product_lang,$id_type_info
	                    		$background_color_degraded_product_lang_old = productsController::showSpecificInformationByProductLangId($id_product_lang,6);
