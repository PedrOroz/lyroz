				...
				if($_SERVER["REQUEST_METHOD"] == "POST")
				{
					if(!isset($_POST['par1']) || !isset($_POST['id_stripe']) || !isset($_POST['value_product_stripe']))
					{
						...
						exit();
							...
							if(!empty($id_product_stripe) && !empty($id_stripe) && !empty($value_product_stripe)){

		                    	require_once($slash.'controllers/functions/productsController.php');

								productsController::updateInformationProductStripe($id_product_stripe,$id_stripe,$value_product_stripe);
							}else{
									$valor = array("estado" => "false",
