<?php
  $bodyConfirmationLogIn = '<!DOCTYPE html>
                ...
                <tr>
                  <td style="padding:30px;background-color:#ffffff;">
                    <h1 style="margin-top:0;margin-bottom:16px;font-size:26px;line-height:32px;font-weight:bold;letter-spacing:-0.02em;text-align:center;">
                      ¡Bienvenid@ a '.(defined('WEBSITE_CMS') ? WEBSITE_CMS : WEBSITE).'!
                    </h1>
                    <p style="margin:0;text-align:center;">
                      Estimad@ <strong>'.(!empty($obj_user->getName_user()) ? $obj_user->getName_user() : $lang_global["Usuario"]).',</strong> ¡Ya puedes disfrutar de una experiencia gastronómica única en su hogar!
                    </p>
                  </td>
                </tr>';

                if(!empty($obj_user->getEmail_user()) || !empty($password_mail)){
$bodyConfirmationLogIn .= '<tr>
...
$bodyConfirmationLogIn .= (!empty($obj_user->getEmail_user()) ? '<p style="margin:0;font-size:14px;line-height:20px;color:#ffffff"><strong>'.$lang_global["Correo electrónico"].':</strong> <a href="mailto:'.$obj_user->getEmail_user().'" target="_blank" style="color: #ffffff;text-decoration:none;">'.$obj_user->getEmail_user().'</a></p>' : '');

$bodyConfirmationLogIn .= (!empty($password_mail) ? '<p style="margin:0;font-size:14px;line-height:20px;color:#ffffff"><strong>'.$lang_global["Contraseña"].':</strong> '.$password_mail.'</p>' : '');

$bodyConfirmationLogIn .= '</td>
                </tr>';
                }
